...
class HETriMesh( object ):
    ...
    class HalfEdge( object ):
        ...
        def print(self):
            ...
            print("opposite_he", self.opposite_he)
            print("next_he", self.next_he)
    ...
    def get_dangling_vertices( self ):
        '''
        Returns vertex indices in HETriMesh 'mesh' that belong to no faces.
        '''
    ...
    def FromTriMeshes( meshes ):
        ...
        Given a sequence of meshes, each with two properties,
            .vs, containing a list of 3d vertices
            .faces, containing a list of triangles as triplets of indices into .vs
        returns a single HETriMesh object containing all meshes concatenated together.
        '''
    ...
    def FromOBJ_FileName( obj_fname ):
        if obj_fname.endswith( '.gz' ):
            ...
            f = open( obj_fname )
        return HETriMesh.FromOBJ_Lines( f )
    ...
    def FromOBJ_Lines( obj_lines ):
        '''
        Given lines from an OBJ file, return a new HETriMesh object.
...
## In order to allow pickling of class HETriMesh, we'll make a reference to the inner HalfEdge class
## here at the module level.
HalfEdge = HETriMesh.HalfEdge

def main():
    import sys
    if len( sys.argv ) > 1:
        mesh = HETriMesh.FromOBJ_FileName( sys.argv[1] )
